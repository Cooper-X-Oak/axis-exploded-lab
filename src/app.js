...
const AXIS_RIG_BLOCKERS = {
  ...
  flow_pos_seat_stack: ["flow_pos_spring_ring"],
...
const fillLight = new THREE.DirectionalLight(0xdde8ec, 0.7);
...
const hemiLight = new THREE.HemisphereLight(0xf2f7f8, 0x1d2527, 0.28);
scene.add(hemiLight);
...
function buildHeroExplodedLayout() {
  ...
  state.motionUnits = state.meshes.map((mesh, index) => {
    ...
    return {
      ...
      dependsOn: [],
      band: roleBand(role),
      anchor: role === "polished_ball" || sourceGroup === "central-body-anchor",
...
function computeRoleLayouts() {
  ...
  roles.forEach((role, index) => {
    ...
    const isAnchor = role === "body_cast_shell";
...
function fitCameraToBox(box) {
  ...
  if (state.viewMode === "hero-exploded") {
    ...
    const distance = Math.max(0.18, fitHeightDistance, fitWidthDistance, depthAllowance) * 1.28;
    camera.up.set(0, 1, 0);
...
function setLightStrength(value) {
  ...
  keyLight.intensity = 2.3 * state.lightStrength;
  fillLight.intensity = 0.7 * state.lightStrength;
  rimLight.intensity = 1.25 * state.lightStrength;
  hemiLight.intensity = 0.28 * state.lightStrength;
